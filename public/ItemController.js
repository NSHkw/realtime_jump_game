...
class ItemController {
  ...
  createItem() {
    const items = this.itemUnlock.find((stage) => stage.id === this.currentStageId);
    const unlockedItems = this.itemImages.filter((data) => items.includes(data.id));

    const index = this.getRandomNumber(0, this.itemImages.length - 1);
    const itemInfo = unlockedItems[index];
